...
def setup_hook(config):
    ...
    def my_get_script_args(dist, executable=sys.executable):
        try:
            # Essayer d'utiliser la fonction originale
            return _main_module()._orig_get_script_args(dist, executable)
        except AttributeError:
            try:
                # Fallback pour setuptools modernes
                from setuptools.command.easy_install import get_script_args
                return get_script_args(dist, executable)
            except ImportError:
                # Dernier recours pour anciennes versions
                return easy_install.get_script_args(dist, executable)
